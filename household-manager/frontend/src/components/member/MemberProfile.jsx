...
export default function MemberProfile() {
  ...
  async function handleSave(e) {
    ...
    if (!currentPassword.trim()) {
      setError('Enter your current password to save changes.'); return;
    }
    ...
    try {
      ...
      login(data.user); // refresh user in context + localStorage
      setCurrentPassword('');
      setNewPassword('');
      setConfirmPassword('');
  ...
  return (
    <div className="fade-in space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Update your name, email, and password</p>
      </div>

      {/* Avatar preview */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex items-center gap-5">
        <div
          className="w-20 h-20 rounded-full flex items-center justify-center text-white text-2xl font-extrabold shadow-sm shrink-0 transition-colors duration-200"
        ...
        <div>
          ...
          <span className="inline-block mt-1 text-xs px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 font-medium">
            👤 Member
          </span>
